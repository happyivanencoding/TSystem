...
class MLPreprocessor:
    # 修改1: 在参数列表中增加了 mode 和 max_date
    def __init__(self, params_principal, params_preprocessing, params_strat, mode="training", max_date=None):
        ...
        self.screen_start_date = params_principal.get('screen_start_date')
    ...
    def _load_data(self):
        screen_agg = pd.read_parquet(self.screen_path)
        df_returns = pd.read_parquet(self.returns_path)
    ...
    def _compute_returns(self, df_initial, horizon, sector_neutral, min_periods_vol = 5):
        ...
        try:
            ...
            if sector_neutral in ['ICB19', 'ICB11']:
                df_sectors = df.reset_index(level=1).groupby(
                    ['Date', f'Sector {sector_neutral}']
                )[['Weight in univ', col_stock]].apply(
                ...
                df_market = df.reset_index(level=1).groupby(['Date'])[['Weight in univ', col_stock]].apply(
                    lambda x: (x['Weight in univ'].dot(x[[col_stock]])) / x['Weight in univ'].sum()
                )
...
##############################################    
    ...
    def preprocess(self):
        ...
        """
        # 1. 读取原始数据
        # 修正: 使用 self.screen_path 而不是 self.screen_agg_path
        print(f"Loading data from {self.screen_path}...")
        screen_agg = pd.read_parquet(self.screen_path)

        # ==============================================================================
        # [优化] Production模式下只保留最近8年数据
        # ==============================================================================
        # 修正: 检查 self.mode 是否存在且为 production
        if hasattr(self, 'mode') and self.mode == "production":
            # 确保 max_date 是 datetime 对象
            end_date = pd.to_datetime(self.max_date)
            # 计算起始日期：当前日期 - 8年
            start_date = end_date - pd.DateOffset(years=8)
            
            print(f"Production Mode Optimization: Truncating data to last 8 years ({start_date.date()} to {end_date.date()})")
            
            # 根据 'Date' 列进行筛选
            screen_agg['Date'] = pd.to_datetime(screen_agg['Date'])
            screen_agg = screen_agg[(screen_agg['Date'] >= start_date) & (screen_agg['Date'] <= end_date)]
            
        # ==============================================================================

        # 对稀疏早期历史的宇宙允许从指定日期开始预处理
        if self.screen_start_date is not None:
            screen_agg['Date'] = pd.to_datetime(screen_agg['Date'])
            screen_start_date = pd.to_datetime(self.screen_start_date)
            screen_agg = screen_agg[screen_agg['Date'] >= screen_start_date]

        # 2. 清洗数据
        screen_clean = self._clean_screen(screen_agg)
        ...
        screen_full_feat.to_parquet(self.df_features_path)
