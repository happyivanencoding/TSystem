# NOTE: This is the productionized download_09 optimizer engine.
# Source archived at: 07_backtest_code/_quarantine_20260701/latest_engine_downloads/download_09_optimizer_reference.py
# Only import robustness, solver fallback and standard-weight conversion were added.
...
def _solve_problem_with_available_mip_solver(prob, scip_options=None, verbose=False):
    """Solve the cvxpy MIP problem with the first compatible installed solver."""
    cvxpy = _require_cvxpy()
    scip_options = scip_options or {}
    installed = set(cvxpy.installed_solvers())
    solver_order = ["SCIP", "HIGHS", "ECOS_BB", "CBC", "GLPK_MI"]
    available = [name for name in solver_order if name in installed]
    if not available:
        raise cvxpy.error.SolverError(
            "No mixed-integer cvxpy solver is installed. Install SCIP, HIGHS, ECOS_BB, CBC or GLPK_MI."
        )

    errors = []
    for solver_name in available:
        kwargs = dict(scip_options) if solver_name == "SCIP" else {}
        try:
            return prob.solve(solver=getattr(cvxpy, solver_name), verbose=verbose, **kwargs)
        except cvxpy.error.SolverError as exc:
            errors.append(f"{solver_name}: {exc}")

    raise cvxpy.error.SolverError("All installed MIP solvers failed. " + " | ".join(errors))
...
def generate_heuristique(result_df,bench,bool_rebal_Europe, bool_rebal_US, init=False):
    ...
    # Mandatory benchmark anchors must not also be forbidden by the small-title ub threshold.
    boites_interdite = [0 if mandatory == 1 else forbidden for forbidden, mandatory in zip(boites_interdite, boite_obligatoire)]
...
# def optimize(result_df, sigma, df_sector_cible, df_pays_cible, bench, current_params, init, scip_options, bool_rebal_Europe, bool_rebal_US, path_output=None):
    ...
    try:
        _solve_problem_with_available_mip_solver(prob, scip_options=scip_options, verbose=False)
...
def to_standard_weight_table(result_df, date_col="Date", id_col="Company SEDOL", weight_col="Wopt"):
    """Convert optimized output to the standard backtest target-weight table."""
    required = [date_col, id_col, weight_col]
    missing = [col for col in required if col not in result_df.columns]
    if missing:
        raise KeyError(f"optimized result missing required columns: {missing}")
    out = result_df.loc[result_df[weight_col].fillna(0) > 0, required].copy()
    out = out.rename(columns={date_col: "Date", id_col: "Company SEDOL", weight_col: "Portfolio weight"})
    out["Date"] = pd.to_datetime(out["Date"])
    out["Company SEDOL"] = out["Company SEDOL"].astype(str)
    out["Portfolio weight"] = pd.to_numeric(out["Portfolio weight"], errors="coerce").fillna(0)
    total = out["Portfolio weight"].sum()
    if total <= 0:
        raise ValueError("optimized result has no positive weight")
    out["Portfolio weight"] = out["Portfolio weight"] / total
    return out
