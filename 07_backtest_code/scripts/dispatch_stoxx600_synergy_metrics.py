...
REPO = Path(r"C:\GoogleDrive\TP")
RUNNER = REPO / "07_backtest_code" / "scripts" / "run_stoxx600_relative_synergy_research.py"
OUT_DIR = REPO / "07_backtest_code" / "runs" / "ad_hoc" / "stoxx600_relative_synergy_20260709"
...
def run_metric(metric: str, output_dir: Path, batch: int, slot: int) -> subprocess.Popen:
    ...
    args = [sys.executable, str(RUNNER), "--workers", "1", "--metrics", metric, "--wave", wave, "--no-pool"]
...
def main() -> int:
    ...
    if not remaining:
        subprocess.run([sys.executable, str(RUNNER), "--workers", "1", "--wave", "wave_20260710_final_summarize"], cwd=str(REPO), check=False)
