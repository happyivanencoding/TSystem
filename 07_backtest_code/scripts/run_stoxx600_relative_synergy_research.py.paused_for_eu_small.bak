...
def pair_column(left: str, right: str) -> str:
    return f"stoxx600_syn_pair_{stable_hash(sorted([left, right]))}"


def subset_column(buckets: tuple[str, ...]) -> str:
    return f"stoxx600_syn_subset_{stable_hash(buckets, 10)}"


def loo_column(bucket: str) -> str:
    return f"stoxx600_syn_loo_without_{base.slugify(bucket)}"
...
def add_candidates(screen: pd.DataFrame, legs: pd.DataFrame) -> tuple[pd.DataFrame, list[base.ModelSpec], pd.DataFrame]:
    ...
    for bucket in BUCKET_ORDER:
        ...
        if not metrics:
            continue
        column = f"stoxx600_syn_bucket_{base.slugify(bucket)}"
