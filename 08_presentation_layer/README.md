...
`artifacts/reports/build_factor_explorer.py` 生成。页面保留四市场每个候选的
...
- `system_dashboard` 默认只读已有 parquet、report 和 manifest；启动写入型工作必须通过既有 pipeline CLI，并由 `artifacts/pipeline_runs/manifests` 留审计证据。
...
- 控制塔审计日志读取 `artifacts/pipeline_runs/manifests`，支持按 step、status、日期范围、as-of 和 input-month 筛选历史运行。
- 控制塔可显式保存当前 pipeline / 子项目运行配置到 `artifacts/dashboard_work/dashboard_config.json`；保存配置不会启动 pipeline，也不会修改生产 parquet。
