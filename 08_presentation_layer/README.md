...
`08_presentation_layer/` 是 TP 的统一展示/报告层。新代码、新启动命令和新报告入口都应优先从这里进入；`08_company_analysis/`、`08_dashboard_analysis/`、`08_web_app_des_companies/` 继续保留为兼容入口和具体实现目录，不再作为三套并行主线维护。
...
| `presentation_layer.apps.des_companies` | Dash 公司展示应用 | `08_web_app_des_companies/src` |
| `presentation_layer.apps.company_analysis_api` | 公司分析 FastAPI | `08_company_analysis/backend/analysis.py` |
| `presentation_layer.reports.portfolio_dashboard` | 组合 dashboard 和 PDF 报告 | `08_dashboard_analysis/dashboard.py`、`pdf_report_generator.py` |

## 维护规则

- 新代码不得直接新建第四套展示/报告入口；需要新增页面或报告时，先接入 `presentation_layer.apps` 或 `presentation_layer.reports`。
- UI/callback/report 可以暂时留在旧目录，但数据读取必须优先走 `PresentationDataRepository`。
- 旧目录的 `app.py`、`backend/main.py` 等入口只保留兼容作用；真实 app factory 在 `presentation_layer.apps`。
