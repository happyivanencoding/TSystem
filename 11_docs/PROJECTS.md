...
| 展示/报告层合并 | 已达成第一版 | `08_presentation_layer` 已成为统一 app/report 入口；Dash 公司展示、公司分析 FastAPI、组合 dashboard/PDF 均通过 `presentation_layer.apps` 或 `presentation_layer.reports` 进入，旧目录保留为实现目录和兼容入口。 |
...
| `08_presentation_layer/` | 展示/报告层统一入口：数据 repository、app factory、report wrapper 和 CLI | [`../08_presentation_layer/README.md`](../08_presentation_layer/README.md) | canonical screen/returns/signals | 展示/报告主线 |
...
| `99_backtest_web_app_legacy/` | 原 Streamlit/FastAPI 回测平台 | [`../99_backtest_web_app_legacy/README.md`](../99_backtest_web_app_legacy/README.md) | canonical screen/returns | 前端和重复核心已隔离，保留历史参考 |
...
| `03_ml_enhanced/Pipeline.ipynb` | 保留 | 当前 ML 训练/预测主 notebook，后续逐步 CLI 化 |
| `03_ml_enhanced/Monitoring.ipynb` | 保留 | 已由 `Monitoring - NEW.ipynb` 合并替换，作为当前监控入口 |
...
| `08_company_analysis/日常分析模板.ipynb` | 保留 | 公司分析日常模板；最新截面读取已接入 `PresentationDataRepository` |
...
1. `backtest_wep_app` 根目录旧 `.pkl` 数据副本已隔离到 `_quarantine_20260629/legacy_data_copies/`；Web/API/Streamlit/Docker 入口已隔离到 `legacy_frontend/`，重复回测核心已隔离到 `legacy_backtest_core/`，主线改为 `07_backtest_code/`。
