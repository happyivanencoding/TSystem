"""Build a self-contained HTML review page from a v2.1 research result directory."""
...
def build_report(results_dir: str | Path, output: str | Path) -> Path:
    ...
    selection = pd.read_csv(results / "model_selection_summary.csv") if (results / "model_selection_summary.csv").exists() else pd.DataFrame()
    coverage = pd.read_csv(results / "coverage_gate_results.csv") if (results / "coverage_gate_results.csv").exists() else pd.DataFrame()
    pit = pd.read_csv(results / "pit_audit.csv") if (results / "pit_audit.csv").exists() else pd.DataFrame()
    alternative = pd.read_parquet(results / "alternative_allocator_results.parquet") if (results / "alternative_allocator_results.parquet").exists() else pd.DataFrame()
    mdd = pd.read_csv(results / "mdd_comparison.csv") if (results / "mdd_comparison.csv").exists() else pd.DataFrame()
    latest = panel.copy()
    ...
    for _, row in factors.iterrows():
        rows.append(
            ...
            f"<td>{escape(str(row.get('score_scale', '0-10')))}</td>"
            f"<td><code>{escape(str(row.get('canonical_formula', '')))}</code></td>"
            f"<td>{escape(str(row.get('definition', '')))}</td>"
            "</tr>"
        )
    html = f"""<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>月度因子推荐 v2.1 | Research Review</title>
<style>
...
<section class="hero"><div class="small">TP Research / Factor Recommendation</div><h1>月度因子推荐 v2.1</h1>
<p>官方因子 sleeve 研究复核页。研究单位为 Date × Region × RegionComponent × Factor × SleeveSide；主目标为 next_month_top_sleeve_net_active_return。</p>
<div class="chips"><span class="chip">{escape(str(manifest.get('promotion_decision', 'RESEARCH_ONLY')))}</span><span class="chip">Champion: {escape(str(manifest.get('champion_model', 'no_valid_model')))}</span><span class="chip">OfficialPortfolioBacktest</span><span class="chip">PIT status: {escape(str(manifest.get('availability_status', 'snapshot_assumption')))}</span><span class="chip">v2 implementation invalidated</span></div></section>
...
<div class="section note"><strong>展示语义：</strong>Exposure snapshot / Not a forecast。未冻结有效 champion 的 forecast 仍为 <code>model_unavailable / NO_VIEW</code>；本页不把暴露分数命名为预测收益。Small Size 与 Size 共用同一 dependency family，均为 0–10 分数。</div>
<section class="section"><h2>每个因子的定义</h2><div class="table-wrap"><table class="data"><thead><tr><th>因子</th><th>标签</th><th>源字段</th><th>分数尺度</th><th>规范公式</th><th>定义</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div></section>
...
<section class="section"><h2>Model selection frequency</h2><div class="table-wrap">{_table(selection, ['model_id','selection_frequency','selection_rate','selection_eligible','baseline_only','is_champion'])}</div></section>
<section class="section"><h2>Coverage diagnostics（Date × Region × Factor）</h2><div class="table-wrap">{_table(coverage, ['Date','region','factor','eligible_universe_rows','valid_factor_rows','factor_row_coverage','eligible_benchmark_weight','valid_factor_benchmark_weight','factor_weight_coverage','raw_benchmark_weight','retained_country_weight','retained_benchmark_coverage','return_available_weight','return_weight_coverage','formation_available','passed'], limit=240)}</div></section>
<section class="section"><h2>PIT mutation audit</h2><div class="table-wrap">{_table(pit, ['check','passed','availability_status','audit_status','evidence'])}</div></section>
<section class="section"><h2>Alternative allocators</h2><div class="table-wrap">{_table(alternative, ['Date','region','allocator_variant','variant_weight','allocator_turnover','allocator_cost','net_return','active_vs_equal_factor','wealth','drawdown'], limit=240)}</div></section>
<section class="section"><h2>MDD comparison</h2><div class="table-wrap">{_table(mdd, ['region','cost_bps','champion_model','mdd_top2_50bps','mdd_equal_factor_50bps','mdd_deterioration_50bps','comparison_basis'])}</div></section>
