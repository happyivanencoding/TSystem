...
def optimize_weights(df, target_secto, target_country, max_weight = 0.15, objective = "min_mvt", margin =0.05):
    ...
    try :
        ...
        # Comptage des NaN dans la colonne "Score"
        nan_count = df["Score"].isna().sum()
        print(f"Nombre de NaN dans la colonne 'Score' : {nan_count}")
        if nan_count >= 1 :
            # Calcule la moyenne (les NaN sont ignorés automatiquement)
            score_mean = df['Score'].mean()

            # Remplace les NaN par cette moyenne
        ...
        # Calcul du score pondéré avant optimisation
        initial_weights = df["Weight"].values
        initial_scores = df["Score"].values
        initial_weighted_score = sum(initial_weights * initial_scores)
        print(f"Score pondéré avant optimisation: {initial_weighted_score}")
        ###
        ...
        constraints.append(w <= max_weight+0.02)
        
        # Secteurs avec marges
        for s, target in target_secto.items():
            # print("Marges")
            idx = df["Secto"] == s
            constraints.append(cp.sum(w[idx]) >= target * (1 - margin))
            constraints.append(cp.sum(w[idx]) <= target * (1 + margin))
        ...
        if objective == "min_mvt" :
            ...
            print("Optim score IA")
            scores = df["Score"].values
            objective = cp.Maximize(cp.sum(cp.multiply(w, scores)))
        ...
        df["Weight"] = w.value
        print("Poids Max Optimisé : ", df["Weight"].max())
        # Calcul du score pondéré après optimisation
        optimized_weights = w.value
        optimized_weighted_score = sum(optimized_weights * initial_scores)
        print(f"Score pondéré après optimisation: {optimized_weighted_score}")

    except Exception as exc:          # capture l’exception
        print("Raté ! : ", exc)
        df.to_parquet("df.parquet")
        raise ValueError("Probleme Optimisation")
...
class PtfBuilder:
    ...
    def filtrage_esg_liste_noire(self, df, date):
        ...
        if self.esg_exclusion != None: 
            if date.year >= 2014 and self.esg_exclusion > 0:  # this will entrer only when if date.year >= 2014 and isinstance(self.score_pivot_esg, float) is FALSE
    ...
    def sec_list_spot(self,screen_agg_monthly=None):
        ...
        # print("Nb de pays avant : " ,df['Exchange Country Name'].unique())
        df = add_country_group(df)
        # print("Nb de pays apres : " ,df['Country Group'].unique())
        weight_secto_bench = df.groupby(' Benchmark ICB Supersector ')['Weight in ' + self.bench].sum() / df['Weight in ' + self.bench].sum()
        print(len(weight_secto_bench))
        weight_pays_bench = df.groupby('Country Group')['Weight in ' + self.bench].sum() / df['Weight in ' + self.bench].sum()
        ...
        for i in range(len(list_score_col)):
            ...
            if isinstance(self.top_mandatory, int) or isinstance(self.top_mandatory, float):
                ...
                print('top n par pays')
                weight_col = 'Weight in ' + self.bench
                group_col = "Country Group"
                # print("Nb de pays apres : " ,df['Country Group'].unique())
                liste_top_mandatory = (df
            ...
            if len(temp_df) != len(df_top):
                raise ValueError(
                                f"Longueur mismatch : len(temp_df)={len(temp_df)} "
                                f"vs len(df_top)={len(df_top)}. "
                                "Arrêt du programme."
                            )
            ...
            temp_df['Weight'] = df_top['Weight in ' + self.bench].values
            ...
            if self.weight_neutral != None: 
                print("Nb de pays apres : " ,temp_df['Country Group'].unique())
                temp_df = optimize_weights(temp_df, weight_secto_bench, weight_pays_bench)
