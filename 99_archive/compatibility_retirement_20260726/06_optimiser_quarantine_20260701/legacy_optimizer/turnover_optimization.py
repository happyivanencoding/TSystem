...
from tp_core.optimisation import turnover
try:
...
def optimize_portfolio_turnover(file_tech, old_portfolio, max_turnover=0.3, 
    ...
    # 计算初始换手率
    initial_turnover = turnover(result_df['Initial_Weight'].values, old_weights_aligned.values)
    print(f"初始换手率: {initial_turnover:.4f}")
    
    # 如果初始换手率已经低于阈值，使用渐进方法逐步调整权重以满足换手率约束
    if initial_turnover <= max_turnover:
        print("初始换手率已低于阈值，使用初始权重")
        ...
        constraints.append({
            'type': 'ineq',
            'fun': lambda x: max_turnover - turnover(x, old_weights_aligned.values)
        })
        ...
        try:
            ...
            if result1.success and turnover(result1.x, old_weights_aligned.values) <= max_turnover:
                best_result = result1
            ...
            if result2.success and turnover(result2.x, old_weights_aligned.values) <= max_turnover:
            ...
            if best_result is None:
                ...
                success = True
                actual_turnover = turnover(optimized_weights, old_weights_aligned.values)
                print(f"使用直接混合法，alpha={best_alpha:.4f}，换手率={actual_turnover:.4f}")
            else:
            ...
            print(f"实际换手率: {actual_turnover:.4f} (目标: <= {max_turnover:.4f})")
            ...
            if output_dir:
                ...
                turnover_report.loc[(turnover_report.index.isin(old_portfolio.index)) & 
                                  (turnover_report['Weight_Change'] < 0), 'Status'] = 'DECREASE'
            ...
            result_df['Weight'] = result_df['Initial_Weight'].values
            result_df['PTF'] = ptf_name
            result_df['Date'] = date
            result_df['Score'] = df_top['Multi Avg Percentile'].values if 'Multi Avg Percentile' in df_top.columns else 0
            result_df['Success'] = 0
            result_df['Turnover'] = turnover(result_df['Weight'].values, old_weights_aligned.values)
...
def test_turnover_optimization():
    ...
    turnover_values = [results[to]['Turnover'].iloc[0] for to in turnover_constraints]
    plt.bar(range(len(turnover_constraints)), turnover_values, tick_label=[f"{to*100}%" for to in turnover_constraints])
    plt.axhline(y=0.3, color='r', linestyle='-', label='30% 目标')
    plt.xlabel('最大换手率约束')
    plt.ylabel('实际换手率')
    plt.title('不同换手率约束下的优化结果')
    plt.savefig(f"{output_dir}/turnover_comparison.png")
    print(f"换手率对比图已保存到 {output_dir}/turnover_comparison.png")
