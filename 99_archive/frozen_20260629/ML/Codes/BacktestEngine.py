...
def merge_ticker_secondaire(df):
        isin_pairs = [
                        "US02079K3059", # Google
                        "US02079K1079",
...
class PtfBuilder:
    ...
    def generic_histo_seclist(self, start_date, freq_rebal=None, screen_start_date = "mois_impair"):
        ...
        *args : 
            First argument is screen_agg (DataFrame or path to pickle file)
        ...
        if type(screen_agg) == str:
            screen_agg = _read_dataframe(screen_agg)
    ...
    def backtest(self,sec_list=None,
        ...
        # if input is path, then read the pickle, if it's a df, then use it directly
        if type(self.screen)==str:
            screen_agg = _read_dataframe(self.screen)
        else:
            screen_agg=copy.deepcopy(self.screen)
    
        if type(self.returns)==str:
            df_returns = _read_dataframe(self.returns)
        else:
