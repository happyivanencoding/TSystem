...
################ FOR EXCEL SHAP ################
def combine_score_shap(config_EU, config_US, config_OTHER):
    
    score_EU = _read_dataframe(config_EU.params_principal['score_ml_path'])
    score_US = _read_dataframe(config_US.params_principal['score_ml_path'])
    score_OTHER = _read_dataframe(config_OTHER.params_principal['score_ml_path'])

    shap_EU = _read_dataframe(config_EU.params_principal['shap_path'])
    shap_US = _read_dataframe(config_US.params_principal['shap_path'])
    shap_OTHER = _read_dataframe(config_OTHER.params_principal['shap_path'])
    ...
    shap_total = pd.concat([shap_US, shap_EU, shap_OTHER], axis=0)
...
def add_raison_repechage(df, path_ptf_world):
    ptf = _read_dataframe(path_ptf_world)
...
def get_factos():
    ...
    screen_EU = _read_dataframe(config_EU.params_principal.get('df_features_path'))
    screen_US = _read_dataframe(config_US.params_principal.get('df_features_path'))
    screen_OTHER = _read_dataframe(config_OTHER.params_principal.get('df_features_path'))
    ...
    screen_original_data = pd.concat([screen_EU, screen_US, screen_OTHER], axis=0)
...
def get_list_exclusion():
    from Config import config_EU, config_US, config_OTHER
    list_exclu_EU = _read_dataframe(config_EU.params_principal['list_exclusion_path'])
    list_exclu_US = _read_dataframe(config_US.params_principal['list_exclusion_path'])
    list_exclu_OTHER = _read_dataframe(config_OTHER.params_principal['list_exclusion_path'])
    ...
    list_exclu_total = pd.concat([list_exclu_EU, list_exclu_US, list_exclu_OTHER], axis=0)
...
def get_ptf_perf(ptf_ticker, bench_ticker,  ptf_worst_ticker=None, start_date="2015-01-01", source="Bloom"):
    if source!="Bloom": # on a également option de récupérer les perf à partir des fichers backtest
        ptf = _read_dataframe(ptf_ticker).to_frame()
...
def get_bench_cols(screen_path, univ):
    screen = _read_dataframe(screen_path)
...
def top_worst_performers(returns_path, basket, n=25):
    returns = _read_dataframe(returns_path)
