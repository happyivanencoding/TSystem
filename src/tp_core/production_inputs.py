...
2. 在 `C:\GoogleDrive\TP` 运行 `python -m tp_core.production_inputs`，对 `incoming` 中的可识别文件进行分类和归档。默认每个内容期间只保留一个 canonical 副本；重复来源会记录在清单里，但不会再次复制。只有迁移历史目录时才使用 `--include-legacy-sources`，只有明确需要保留所有重复物理文件时才使用 `--keep-duplicates`。
