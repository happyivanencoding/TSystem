...
def detect_high_low(df,window=3):
    """
        Create roll over High, Low and Close price according to a rolling window, this will be used later in the other functions.
    """
    roll_window=window
...
def detect_head_shoulder(df, window=3):
    ...
    df['head_shoulder_pattern']=df['head_shoulder_pattern'].shift(1)
    return df 
...
def calculate_support_resistance(df, window=12):
    roll_window = window
...
def detect_double_top_bottom(df, window=3, threshold=0.05):
    # Define the rolling window and  a threshold to check for the range of pattern
    roll_window = window
    ...
    df['double_pattern'] = df[ 'double_pattern'].shift(1)
    return df


def detect_trendline(df, window=4):
    # Create new columns for the linear regression slope and intercept of closing price on time
    df['slope'] = np.nan
    df['intercept'] = np.nan
    ...
    df['support_bis'] = np.nan
    df['resistance_bis'] = np.nan

    # Populate the new columns using the boolean masks
    df.loc[mask_support, 'support_bis'] = df['Close'] * df['slope'] + df['intercept']
    df.loc[mask_resistance, 'resistance_bis'] = df['Close'] * df['slope'] + df['intercept']
    return df
