"""
Parallel launcher for EU Small relative-variable official backtests.

Each worker owns a disjoint metric shard and writes a separate result CSV. The
parent process merges shard results into the main research directory and then
rebuilds summary, gates, comparison tables, plots, report, and manifest.
"""
...
from tp_research.workflows import run_eu_small_relative_variable_research as rel  # noqa: E402
...
def load_completed(results_paths: list[Path]) -> pd.DataFrame:
    frames = []
...
def worker_run(payload: dict[str, object]) -> dict[str, object]:
    ...
    existing_frames = []
    if existing_results_path.exists() and existing_results_path.stat().st_size > 0:
        try:
            existing_frames.append(pd.read_csv(existing_results_path))
        except pd.errors.EmptyDataError:
            pass
    if shard_results_path.exists() and shard_results_path.stat().st_size > 0:
        try:
            existing_frames.append(pd.read_csv(shard_results_path))
        except pd.errors.EmptyDataError:
            pass
    ...
    columns = list(dict.fromkeys(required + metrics))
    screen = pd.read_parquet(screen_path, columns=columns)
    returns = base.load_tabular_file(returns_path)
    returns.index = pd.to_datetime(returns.index, errors="coerce")
    returns = returns.sort_index()
    # Keep path short while still making restart waves independent.
    wave_code = "".join(ch for ch in wave if ch.isalnum())[-10:] or "manual"
    run_root_name = f"ad_hoc/eurv{wave_code}_s{shard_id:02d}"
    ...
    results.to_csv(shard_results_path, index=False, encoding="utf-8-sig")
    return {
        "shard_id": shard_id,
        "metrics": len(metrics),
        "rows": len(results),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parallel official backtests for EU Small relative variables.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    screen_path = output_dir / "eu_small_relative_variable_screen.parquet"
    ...
    run_results.to_csv(main_results_path, index=False, encoding="utf-8-sig")
    ...
    manifest.update(
        {
            ...
            "parallel_shard_size": max(args.shard_size, 0),
            "parallel_fresh_process_per_batch": bool(args.fresh_process_per_batch),
            "parallel_wave": wave,
