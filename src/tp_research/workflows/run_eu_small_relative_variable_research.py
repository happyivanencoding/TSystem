"""
Build and officially backtest MSCI Europe Small relative-change variables.

This runner treats each same-security relative variant as a new raw variable:
directional level delta and neutral-score delta at lags 1, 3, and 12 screen
observations. It intentionally does not include growth, revision, momentum,
return, CAGR, or other change-like fields in the mechanical relative expansion.
"""
...
DEFAULT_LEVEL_GATE = AD_HOC_ROOT / "eu_small_validated_gate_20260708_official" / "raw_validation_gate.csv"
CHANGE_LIKE_TOKENS = (
    "growth",
    "revision",
    "pmom",
...
def is_change_like(spec: base.RawMetricSpec) -> bool:
    text = f"{spec.column} {spec.note} {spec.family}".lower()
    return any(token in text for token in CHANGE_LIKE_TOKENS)
...
def relative_column(spec: base.RawMetricSpec, transform: str, lag: int) -> str:
    ...
    return f"eu_small_{prefix}_{base.slugify(spec.family)}_{base.slugify(spec.column)}_lag{lag}_score"
...
def economic_read(spec: base.RawMetricSpec, transform: str, lag: int) -> str:
    ...
    if spec.direction > 0:
        return f"level improvement over {lag} screen observations"
...
def build_relative_screen(
    ...
    *,
    ...
    output_path = output_dir / "eu_small_relative_variable_screen.parquet"
            ...
            pd.DataFrame(
                [
                    ...
                    {"check": "transform_types", "value": ",".join(transforms)},
                    {
                        ...
                        "value": "same-ISIN lagged observation; current minus lag after higher-is-better direction normalization",
                    },
                    {
                        "check": "relative_exclusion_rule",
                        "value": "no mechanical relative transforms for growth/revision/momentum/return/CAGR/change-like fields",
                    },
                ]
            ),
        ],
...
def compare_with_level_gate(relative_gate: pd.DataFrame, level_gate_path: Path, output_dir: Path) -> pd.DataFrame:
    ...
    if "raw_column" not in level.columns:
        definitions = [
            {
                "metric": spec.score_column,
                "raw_column": spec.column,
                "base_family": spec.family,
            }
            for spec in base.RAW_METRICS
        ]
        level = level.merge(pd.DataFrame(definitions), on="metric", how="left")
    ...
    out.to_csv(output_dir / "relative_vs_level_comparison.csv", index=False, encoding="utf-8-sig")
...
def write_report(
    *,
    ...
    lines = [
        "# 欧洲小盘股相对 raw variable 官方回测研究",
        "",
        f"- 运行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"- Universe / Benchmark: `{base.BENCHMARK}`",
        "- 证据口径: official exact Top/Worst backtest",
        "- 研究问题: 绝对水平变量是否应转换为同一股票上一期相同变量的相对变化后再作为新 raw variable 测试。",
        f"- 研究目录: `{output_dir}`",
        ...
        "## 官方回测运行状态",
        "",
        base.frame_to_markdown(run_results[["metric", "side", "start_date", "status", "message", "run_dir"]], max_rows=300)
        if not run_results.empty
        else "暂无回测运行。",
        "",
        "## Relative Gate 结果",
        "",
        base.frame_to_markdown(gate, max_rows=160) if not gate.empty else "暂无 gate 结果。",
        "",
        "## 稳健性排序 Top 变量",
        "",
    ]
    if top_summary.empty:
        ...
        lines.append(base.frame_to_markdown(merged[[col for col in display_cols if col in merged.columns]], max_rows=100))

    lines.extend(["", "## 与原始水平变量对照", ""])
    if comparison.empty:
        lines.append("未生成对照表；可能没有提供上一轮 raw-level gate 文件。")
    else:
        display_cols = [
            "raw_column",
            "family",
        ...
        lines.append(base.frame_to_markdown(comparison[[col for col in display_cols if col in comparison.columns]], max_rows=100))
    ...
    if passed.empty:
        lines.extend(
            [
                ...
                "- 因此相对变化只能作为诊断信号继续观察，不能进入 family composite，也不能据此声称协同效应。",
            ]
        )
    else:
        ...
        lines.extend(
            [
                ...
                "- 这些变量必须和其他已通过 raw/relative gate 的变量做 pair / subset / leave-one-out 后，才可声称协同效应。",
            ]
        )
    lines.extend(["", "## Plotly 输出", ""])
    lines.extend([f"- `{path}`" for path in plot_paths] or ["- 未生成 Plotly 输出。"])
    report_path = output_dir / "eu_small_relative_variable_report.md"
    report_path.write_text("\n".join(lines), encoding="utf-8")
    return report_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build and backtest MSCI Europe Small relative-change variables.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else AD_HOC_ROOT / f"eu_small_relative_variables_{timestamp}"
    ...
    research_screen_path = output_dir / "eu_small_relative_variable_screen.parquet"
    checks.to_csv(output_dir / "data_construction_checks.csv", index=False, encoding="utf-8-sig")
    metric_diag.to_csv(output_dir / "metric_diagnostics.csv", index=False, encoding="utf-8-sig")
    ...
    if not args.build_only:
        ...
        run_results.to_csv(output_dir / "official_run_results.csv", index=False, encoding="utf-8-sig")
        ...
        gate.to_csv(output_dir / "relative_validation_gate.csv", index=False, encoding="utf-8-sig")
