"""
Parallel launcher for Nasdaq relative-variable official Top/Worst backtests.

Each worker owns a disjoint metric shard, writes an independent shard CSV, and
uses a wave-specific official run root. Restarts merge all prior waves and only
launch incomplete Top/Worst pairs.
"""
...
from tp_research.workflows import run_nasdaq_relative_variable_research as rel  # noqa: E402
...
def wave_run_root(wave: str, shard_id: int) -> str:
    wave_slug = base.slugify(wave)[-18:]
    return f"ad_hoc/nrv_{wave_slug}_s{shard_id:02d}"
...
def worker_run(payload: dict[str, object]) -> dict[str, object]:
    ...
    results = base.run_official_backtests(
        ...
        run_root_name=wave_run_root(wave, shard_id),
        metrics=metrics,
        max_runs=None,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parallel official backtests for Nasdaq relative variables.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    screen_path = output_dir / "nasdaq_relative_variable_screen.parquet"
