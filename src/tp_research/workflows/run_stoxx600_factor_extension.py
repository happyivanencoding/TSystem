"""STOXX Europe 600 candidate extension with strict official evidence gates."""
...
from tp_research.workflows import run_stoxx600_relative_synergy_research as ext  # noqa: E402
...
DEFAULT_RAW_DIR = AD_HOC_ROOT / "stoxx600_raw_gated_20260708_0100"
DEFAULT_RELATIVE_DIR = AD_HOC_ROOT / "stoxx600_relative_variables_20260709"
DEFAULT_OLD_SYNERGY = AD_HOC_ROOT / "stoxx600_relative_synergy_20260709"
DEFAULT_OUTPUT = AD_HOC_ROOT / "stoxx600_factor_extension_20260711"
...
def country_sector_score(screen: pd.DataFrame, values: pd.Series) -> pd.Series:
    ...
    group = frame.groupby(["date", "country", "sector"], observed=True)["value"]
...
def residual_signals(screen: pd.DataFrame, returns_path: Path) -> pd.DataFrame:
    ...
    for date, month in screen.groupby(base.DATE_COL, sort=True, observed=True):
        ...
        values = pd.DataFrame(
            {
                base.SEDOL_COL: ids,
                "stoxx_residual_momentum_raw": momentum,
                "stoxx_residual_momentum_risk_adjusted_raw": risk_adjusted,
                "stoxx_residual_volatility_raw": vol,
                "stoxx_residual_downside_volatility_raw": downside,
            }
        )
...
def build_extension_screen(
    ...
    screen = pd.read_parquet(raw_dir / "stoxx600_multifactor_screen.parquet")
    extra_columns = [
        base.DATE_COL,
        base.ISIN_COL,
        COUNTRY_COL,
    ...
    component_scores: dict[str, str] = {
        "Sales Growth FY1": "stoxx600_growth_sales_growth_fy1_score",
        "Gross Income Growth FY1": "stoxx600_growth_gross_income_growth_fy1_score",
        "EPS Revision Ratio": "stoxx600_momentum_eps_revision_ratio_score",
        "PMOM 12M1M": "stoxx600_momentum_pmom_12m1m_score",
        "Oper Margin": "stoxx600_quality_oper_margin_score",
        "NetDebt to EBITDA exFIN": "stoxx600_quality_netdebt_to_ebitda_exfin_score",
        "Earns Yield FY1": "stoxx600_value_earns_yield_fy1_score",
        "FCF Conversion": "stoxx600_quality_fcf_conversion_score",
        "DPS 1Y Growth FY1": "stoxx600_dividend_dps_1y_growth_fy1_score",
    }
    ...
    for output, source, direction, lag in country_specs:
        ...
        screen[output] = country_sector_score(screen, values)
        component_scores[output] = output

    screen["asset_turnover_score"] = sector_score(screen, "Asset TO exFIN", 1)
    screen["country_asset_turnover_score"] = country_sector_score(screen, screen["Asset TO exFIN"])
    for lag in (1, 3):
        delta = pd.to_numeric(screen["Asset TO exFIN"], errors="coerce")
        delta = delta - delta.groupby(entity).shift(lag)
        column = f"country_asset_turnover_delta{lag}_score"
        screen[column] = country_sector_score(screen, delta)
        component_scores[column] = column
    ...
    for column in ("fcf_sales_raw", "rd_intensity_raw", "rd_adjusted_ebit_margin_raw"):
        score_column = f"{column}_score"
        screen[score_column] = sector_score(screen, column, 1)
    ...
    for column, direction in {
        "stoxx_residual_momentum_raw": 1,
        "stoxx_residual_momentum_risk_adjusted_raw": 1,
        "stoxx_residual_volatility_raw": -1,
        "stoxx_residual_downside_volatility_raw": -1,
    }.items():
        score_column = f"{column}_score"
    ...
    definitions = [
        ("stoxx_ext_country_revision", "Country-aware EPS Revision", "revision", ["country_eps_revision_raw"], 1, "EPS revisions ranked inside sufficiently broad country-sector cells."),
        ("stoxx_ext_country_pmom", "Country-aware PMOM", "pmom", ["country_pmom_raw"], 1, "Price momentum with country-sector exposure control."),
        ("stoxx_ext_country_margin_improvement", "Country-aware Margin Improvement", "quality_improvement", ["country_oper_margin_delta3_raw"], 1, "Three-observation operating-margin improvement with country awareness."),
        ("stoxx_ext_country_deleveraging", "Country-aware Deleveraging", "deleveraging", ["country_deleveraging_delta3_raw"], 1, "Three-observation NetDebt/EBITDA improvement with country awareness."),
        ("stoxx_ext_country_earnings_yield_improvement", "Country-aware Earnings-yield Improvement", "earnings_yield_improvement", ["country_earnings_yield_delta1_raw"], 1, "One-observation earnings-yield improvement with country awareness."),
        ("stoxx_ext_country_quality_repair", "Country-aware Quality Repair", "quality_improvement", ["country_oper_margin_delta3_raw", "country_deleveraging_delta3_raw"], 2, "Margin improvement jointly confirmed by deleveraging."),
        ("stoxx_ext_country_value_confirmation", "Country-aware Value Confirmation", "earnings_yield_improvement", ["country_earnings_yield_delta1_raw", "country_eps_revision_raw"], 2, "Earnings-yield improvement confirmed by revisions."),
        ("stoxx_ext_asset_turnover", "Asset Turnover", "capital_efficiency", ["asset_turnover_score"], 1, "Sales productivity of the non-financial asset base."),
        ("stoxx_ext_country_asset_turnover", "Country-aware Asset Turnover", "capital_efficiency", ["country_asset_turnover_score"], 1, "Asset productivity with country-sector exposure control."),
        ("stoxx_ext_asset_turnover_improvement_1m", "Asset Turnover Improvement 1M", "capital_efficiency", ["country_asset_turnover_delta1_score"], 1, "One-observation asset-turnover improvement."),
        ("stoxx_ext_asset_turnover_improvement_3m", "Asset Turnover Improvement 3M", "capital_efficiency", ["country_asset_turnover_delta3_score"], 1, "Three-observation asset-turnover improvement."),
        ("stoxx_ext_capital_efficient_growth_revision", "Capital-efficient Growth + Revision", "capital_efficiency", ["Sales Growth FY1", "Gross Income Growth FY1", "country_asset_turnover_score", "EPS Revision Ratio"], 3, "Growth confirmed by asset productivity and revisions."),
        ("stoxx_ext_cash_productivity", "FCF / Sales", "capital_efficiency", ["fcf_sales_raw"], 1, "Free-cash-flow productivity relative to sales."),
        ("stoxx_ext_rd_intensity", "R&D Intensity", "intangible_investment", ["rd_intensity_raw"], 1, "R&D expense divided by sales; missing R&D is not imputed."),
        ("stoxx_ext_rd_adjusted_margin", "R&D-adjusted EBIT Margin", "intangible_investment", ["rd_adjusted_ebit_margin_raw"], 1, "EBIT plus current R&D divided by sales; diagnostic accounting adjustment."),
        ("stoxx_ext_residual_momentum", "STOXX-residual Momentum 12-1", "residual_momentum", ["stoxx_residual_momentum_raw"], 1, "12-1 momentum after removing rolling STOXX benchmark beta."),
        ("stoxx_ext_residual_momentum_risk_adjusted", "Risk-adjusted Residual Momentum", "residual_momentum", ["stoxx_residual_momentum_risk_adjusted_raw"], 1, "Residual momentum scaled by residual volatility."),
        ("stoxx_ext_residual_momentum_revision", "Residual Momentum + Revision", "residual_momentum", ["stoxx_residual_momentum_raw", "EPS Revision Ratio"], 2, "Residual price continuation confirmed by earnings revisions."),
        ("stoxx_ext_low_residual_volatility", "Low Residual Volatility", "residual_risk", ["stoxx_residual_volatility_raw"], 1, "Low company-specific volatility after removing STOXX beta."),
        ("stoxx_ext_low_residual_downside_risk", "Low Residual Downside Risk", "residual_risk", ["stoxx_residual_downside_volatility_raw"], 1, "Low downside company-specific volatility."),
        ("stoxx_ext_downside_aware_revision", "Downside-aware Revision", "residual_risk", ["EPS Revision Ratio", "stoxx_residual_downside_volatility_raw"], 2, "Earnings revisions penalized by residual downside risk."),
        ("stoxx_ext_dividend_sustainability", "Dividend Sustainability", "dividend_sustainability", ["DPS 1Y Growth FY1", "low_payout_score", "FCF Conversion"], 2, "Dividend growth supported by payout discipline and cash conversion where available."),
        ("stoxx_ext_dividend_growth_deleveraging", "Dividend Growth + Deleveraging", "dividend_sustainability", ["DPS 1Y Growth FY1", "NetDebt to EBITDA exFIN"], 2, "Dividend growth supported by balance-sheet strength."),
        ("stoxx_ext_dividend_revision_quality", "Dividend Growth + Revision + Payout", "dividend_sustainability", ["DPS 1Y Growth FY1", "EPS Revision Ratio", "low_payout_score"], 3, "Dividend growth confirmed by revisions and payout discipline."),
    ]
    ...
    screen.to_parquet(output_dir / "stoxx600_extension_screen.parquet", index=False)
...
def build_theme_matrix(
    ...
    old_columns = {theme: f"stoxx600_syn_bucket_{base.slugify(theme)}" for theme in OLD_CORE_THEMES}
    id_columns = [base.DATE_COL, base.ISIN_COL, base.SEDOL_COL, "Name", base.SECTOR_COL, base.MKT_CAP_COL, base.WEIGHT_COL]
    old = pd.read_parquet(old_synergy_dir / "stoxx600_relative_synergy_screen.parquet", columns=[*id_columns, *old_columns.values()])
    ...
    for theme in themes:
        ...
        if not components:
            continue
        column = f"stoxx600_ext_theme_{base.slugify(theme)}"
    ...
    for size in (2, 3):
        for combo in combinations(theme_columns, size):
            column = f"stoxx600_ext_subset_{hashlib.sha1('|'.join(combo).encode('utf-8')).hexdigest()[:12]}"
    ...
    screen.to_parquet(output_dir / "stoxx600_extension_theme_screen.parquet", index=False)
...
def write_report(
    ...
    lines = [
        "# STOXX Europe 600 因子候选扩展研究",
        "",
        f"- 生成时间：{datetime.now().isoformat(timespec='seconds')}",
        "- 证据：official exact Top/Worst；Gate 与旧研究相同。",
        "- 范围：国家感知、资本效率、无形资产、残差动量/风险、股息持续性。",
        "",
        "## 结论",
        "",
        f"24 个预注册候选中 {len(passed)} 个通过 Gate；通过后形成 {manifest.get('validated_theme_count', 0)} 个有效主题。",
        ...
        "## 外部研究锚点",
        "",
        "- [STOXX Europe 600 官方页](https://stoxx.com/index/sxxgr/)",
        "- [Fama–French 国际五因子](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2622782)",
        "- [Value and Momentum Everywhere](https://onlinelibrary.wiley.com/doi/10.1111/jofi.12021)",
        "- [Quality Minus Junk](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2312432)",
        "- [Novy-Marx Gross Profitability](https://www.nber.org/papers/w15940)",
        "- [Betting Against Beta](https://www.sciencedirect.com/science/article/pii/S0304405X13002675)",
    ]
    path = output_dir / "stoxx600_factor_extension_report.md"
    path.write_text("\n".join(lines), encoding="utf-8")
    return path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="STOXX600 factor candidate extension.")
...
def main() -> int:
    ...
    if not args.build_only:
        gate_results = run_parallel(output_dir / "gate_runs", output_dir / "stoxx600_extension_screen.parquet", Path(args.returns), [spec.column for spec in specs], max(1, args.workers), datetime.now().strftime("gate_%H%M%S"))
        ...
        if not args.gate_only:
            ...
            theme_results = run_parallel(output_dir / "theme_runs", output_dir / "stoxx600_extension_theme_screen.parquet", Path(args.returns), [spec.column for spec in theme_specs], max(1, args.workers), datetime.now().strftime("theme_%H%M%S"))
            ...
            _, subset, loo, claims = ext.summarize_synergy(theme_summary, empty_legs, candidate_map, output_dir)
