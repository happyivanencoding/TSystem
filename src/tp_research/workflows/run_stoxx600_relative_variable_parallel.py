"""
Parallel launcher for STOXX Europe 600 relative-variable official backtests.

Each worker owns a disjoint metric shard and writes a separate result CSV. The
parent process merges shard results back into the main research directory and
rebuilds summary, gate, comparison, plots, and report.
"""
...
def worker_run(payload: dict[str, object]) -> dict[str, object]:
    ...
    launched = len(results[~results["status"].isin(["success", "skipped"])]) if not results.empty else 0
    return {
        "shard_id": shard_id,
        "metrics": len(metrics),
        "rows": len(results),
        ...
        "launched_nonterminal": int(launched),
    }
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    screen_path = output_dir / "stoxx600_relative_variable_screen.parquet"
