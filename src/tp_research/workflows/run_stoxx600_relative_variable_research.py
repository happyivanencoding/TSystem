"""
Build and officially backtest STOXX Europe 600 relative-change variables.

The research question is whether absolute level variables become more useful
when converted into point-in-time changes versus the same variable's prior
observation for the same security.
"""
...
DEFAULT_LEVEL_GATE = AD_HOC_ROOT / "stoxx600_raw_gated_20260708_0100" / "raw_validation_gate.csv"
CHANGE_LIKE_TOKENS = ("growth", "revision", "pmom", "total return", "cagr")
OUTPUT_PREFIX = "stoxx600"
REPORT_TITLE = "STOXX Europe 600 相对变量官方回测研究"
DEFAULT_OUTPUT_PREFIX = "stoxx600_relative_variables"
...
def relative_column(spec: base.RawMetricSpec, transform: str, lag: int) -> str:
    ...
    return f"{OUTPUT_PREFIX}_{prefix}_{base.slugify(spec.family)}_{base.slugify(spec.column)}_lag{lag}_score"
...
def build_relative_screen(
    ...
    *,
    ...
    output_path = output_dir / f"{OUTPUT_PREFIX}_relative_variable_screen.parquet"
...
def economic_read(spec: base.RawMetricSpec, transform: str, lag: int) -> str:
    direction_text = "improvement" if spec.direction > 0 else "decline in a lower-is-better level"
    if transform == "score_delta":
        return f"relative peer-rank improvement over {lag} screen observations"
    return f"{direction_text} over {lag} screen observations"
...
def compare_with_level_gate(relative_gate: pd.DataFrame, level_gate_path: Path, output_dir: Path) -> pd.DataFrame:
    ...
    if "raw_column" not in level.columns:
        metric_to_raw = {spec.score_column: spec.column for spec in base.RAW_METRICS}
        level["raw_column"] = level.get("metric", pd.Series(dtype=str)).map(metric_to_raw)
    if "pass_gate" not in level.columns and "passed" in level.columns:
        level["pass_gate"] = level["passed"].astype(str).str.lower().eq("true")
    level = level.dropna(subset=["raw_column"]).copy()
    if level.empty:
        return pd.DataFrame()
    ...
    rel_best = rel_best[[col for col in cols if col in rel_best.columns]].rename(
        columns={
...
def write_report(
    *,
    ...
    lines = [
        ...
        "- 研究问题: 绝对水平变量是否应转换为同一股票上一期相同变量的相对变化后再作为新变量测试",
        f"- 研究目录: `{output_dir}`",
        ...
        "## 相对变量定义与覆盖",
        "",
        base.frame_to_markdown(metric_diag.sort_values(["base_family", "raw_column", "transform", "lag_observations"]), max_rows=240)
        if not metric_diag.empty
        else "暂无变量定义。",
        "",
        "## 官方回测运行状态",
        "",
        base.frame_to_markdown(run_results[["metric", "side", "start_date", "status", "message", "run_dir"]], max_rows=260)
        if not run_results.empty
        else "暂无回测运行。",
        "",
        "## Gate 结果",
        "",
        base.frame_to_markdown(gate, max_rows=140) if not gate.empty else "暂无 gate 结果。",
        "",
        "## 稳健性排序 Top 变量",
        "",
    ]
    if top_summary.empty:
        ...
        lines.append(base.frame_to_markdown(merged[[col for col in display_cols if col in merged.columns]], max_rows=80))
    ...
    if comparison.empty:
        ...
        lines.append(base.frame_to_markdown(comparison[[col for col in display_cols if col in comparison.columns]], max_rows=80))
    ...
    if passed.empty:
        lines.extend(
            [
                ...
                "- 这意味着相对变化可以作为诊断信号继续观察，但不能仅凭本轮证据进入 family composite。",
            ]
        )
    else:
        ...
        lines.extend(
            [
                ...
                "- 这些变量仍需和其他已通过 raw gate 的变量做 pair / subset / leave-one-out 后，才可声称协同效应。",
            ]
        )
    lines.extend(["", "## Plotly 输出", ""])
    lines.extend([f"- `{path}`" for path in plot_paths] or ["- 未生成 Plotly 输出。"])
    report_path = output_dir / f"{OUTPUT_PREFIX}_relative_variable_report.md"
    report_path.write_text("\n".join(lines), encoding="utf-8")
    return report_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build and backtest STOXX Europe 600 relative-change variables.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else AD_HOC_ROOT / f"{DEFAULT_OUTPUT_PREFIX}_{timestamp}"
    ...
    research_screen_path = output_dir / f"{OUTPUT_PREFIX}_relative_variable_screen.parquet"
