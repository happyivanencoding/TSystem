...
def test_repository_exposes_canonical_paths() -> None:
    ...
    assert repo.signals_dir.name == "signals"
...
def test_system_jobs_runner_writes_and_reads_records(tmp_path: Path) -> None:
    ...
    record = system_jobs.launch_job(
        ...
        "project:candidates:safe_check",
        tmp_path / "launches",
        tmp_path,
        popen_factory=lambda *args, **kwargs: FakeProcess(),
        creationflags=0,
    )
    assert record["job_id"].startswith("project_candidates_safe_check_")
...
""",
    ...
    assert any(row["项目"] == "tp_core" and row["状态"] == "未检查" for row in check_rows)
    ...
    assert _is_ignored_asset(Path("artifacts/signals/ml_signals.parquet"))
    project_asset_summary = _project_asset_summary_rows(
        [
            ...
            {"项目": "signals", "来源": "registry", "状态": "存在", "必需": "是", "数据/产物": "ml_signals", "_bytes": 2048, "_mtime": 3000},
        ]
    )
    ...
    assert project_summary_by_id["signals"]["资产状态"] == "OK"
    sample_assets = [
    ...
    assert loaded["project_id"] == "candidates"
    ...
    monkeypatch.setattr(dashboard.subprocess, "Popen", lambda *args, **kwargs: FakeProcess())
    launch_record = _launch([sys.executable, "-c", "print('ok')"], "project:candidates:safe_check")
    Path(launch_record["log_path"]).write_text("starting\ncompleted marker\n", encoding="utf-8")
    assert launch_record["job_id"].startswith("project_candidates_safe_check_")
    ...
    assert len([row for row in launches if row["step"] == "project:candidates:safe_check"]) == 1
    latest_project_launch = _latest_project_launch("candidates")
    ...
    assert _project_card_button_id("candidates", "registered_command") == {
        "type": "tp-project-card-select",
        "project": "candidates",
        "mode": "registered_command",
    }
    assert _project_card_selection(_project_card_button_id("candidates", "safe_check")) == (
        "candidates",
        "safe_check",
    )
    ...
    registered_command = _build_project_command("candidates", "registered_command")
    ...
    assert any(project["project_id"] == "candidates" for project in lineage["projects"])
